...
class RocketChatDriver
	...
	setupReactiveOnlineUser: (robotname)=>
		...
		@messagesOnlineUser = @asteroid.getCollection 'users'
		
		rQ = @messagesOnlineUser.reactiveQuery {}
		#console.log('result:', JSON.stringify(rQ.result, null, 2))
		...
		rQ.on "change", (id) =>
			...
			# Some other publish calls is polluting local user collection for the first login.
			# hence applying utfOffset field as a filter to sent that message only once.

			changedMsgQuery = @messagesOnlineUser.reactiveQuery {"_id": id , "status":"online"}
			
			if changedMsgQuery.result && changedMsgQuery.result.length > 0 
				if (changedMsgQuery.result[0].hasOwnProperty("utcOffset"))
					console.log('result:', JSON.stringify(changedMsgQuery.result, null, 2))
					changedMsg = changedMsgQuery.result[0]
					botmessage= "Hi! " + changedMsg.username + " Iam a Bot and my name is "+ robotname	
	
					message="``` \n"
					message+= "    (\\____/)             |---------------------------------------------\n"
					message+= "     (_oo_) -------------|"+botmessage+"\n"
					message+= "       (O)               |---------------------------------------------\n"
					message+= "     __||__    \\)\n"
					message+= "  []/______\\[] /\n"
					message+= "  / \______/ \\/\n"
					...
					r = @getDirectMessageRoomId changedMsg.username
					r.then (room) =>
